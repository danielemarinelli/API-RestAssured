...
@Epic("Epic-01")
@Feature("Create Update Delete Pizzas for restautants")
public class AllureReportGeneration extends BaseTest {
	...
	public void e2eAPIRequest() {
		...
		try {
			...
			//put api call
			RestAssured
				.given().filter(new RestAssuredListener()).filter(new AllureRestAssured())
					.contentType(ContentType.JSON)
			...
			// training 3h 55min TESTNG SUITE!!!!
		} catch (IOException e) {
	...
	@Story("Story 1")
	@Test(description = "delete record in DB")
